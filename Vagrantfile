...
:inetRouter => {
  ...
  :centralRouter => {
        ...
        :net => [
                   ...
                   {ip: '192.168.0.33', adapter: 4, netmask: "255.255.255.240", virtualbox__intnet: "officehardware"},
  ...
  :centralServer => {
        :box_name => "centos/7",
        :net => [
                   {ip: '192.168.0.2', adapter: 2, netmask: "255.255.255.240", virtualbox__intnet: "dirrectors"}
                ]
  },
  ...
  :office1Router => {
    ...
    :net => [
               ...
               {ip: '192.168.2.1', adapter: 3, netmask: "255.255.255.192", virtualbox__intnet: "dev-1-net"},
               {ip: '192.168.2.65', adapter: 4, netmask: "255.255.255.192", virtualbox__intnet: "testservers-1-net"},
               {ip: '192.168.2.129', adapter: 5, netmask: "255.255.255.192", virtualbox__intnet: "managers-net"},
               {ip: '192.168.2.193', adapter: 6, netmask: "255.255.255.192", virtualbox__intnet: "hardware-1-net"}
            ]
  },
  ...
  :office1Server => {
    :box_name => "centos/7",
    :net => [
               {ip: '192.168.2.2', adapter: 2, netmask: "255.255.255.192", virtualbox__intnet: "dev-1-net"}
            ]
  },
  ...
  :office2Router => {
    ...
    :net => [
              ...
              {ip: '192.168.1.1', adapter: 3, netmask: "255.255.255.128", virtualbox__intnet: "dev-2-net"},
              {ip: '192.168.1.129', adapter: 4, netmask: "255.255.255.192", virtualbox__intnet: "testservers-2-net"},
              {ip: '192.168.1.193', adapter: 5, netmask: "255.255.255.192", virtualbox__intnet: "hardware-2-net"}
            ]
  }, 
    ...
    config.vm.define boxname do |box|
        ...
        when "centralRouter"
          box.vm.provision "shell", run: "always", inline: <<-SHELL
            ...
            sudo bash -c 'echo "192.168.2.0/26 via 192.168.254.2 dev eth1" > /etc/sysconfig/network-scripts/route-eth1'
            sudo bash -c 'echo "192.168.1.0/25 via 192.168.253.2 dev eth1" >> /etc/sysconfig/network-scripts/route-eth1'

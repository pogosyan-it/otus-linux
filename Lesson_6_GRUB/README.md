...
2. **Переименовать VG, содержащем на одном из своих логических томов каталог /root**
Эта задача автоматизирована с помощью скрипта:
https://github.com/pogosyan-it/otus-linux/blob/master/Lesson_6_GRUB/vg-rename.sh
Скрипт сам определяет ту группу томов на которой находится каталог /root и переименовывает его в OtusNewRoot, правит fstab, /etc/default/grub, монтирует рутовый логический том в  /mnt, после чего chroot в /mnt, так как если этого не сделать то пересобрать невозможно - не находит по новому названию vg в /root/

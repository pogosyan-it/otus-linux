...
Записи журнала транзакций можно взять с одного сервера базы данных – ведущего – и применить к файлам данных на другом сервере – ведомом. В таком случае ведомый сервер будет иметь точную копию базы данных на ведущем сервере. Процесс передачи записей из журнала транзакций на другой сервер и их применения называется физической репликацией. Слово «физическая» означает, что журнал транзакций работает на низком уровне и реплика базы данных на ведомом сервере будет точной побайтовой копией базы данных на ведущем сервере.

Физическая репликация работает для всех баз данных в кластере. Создание новой базы отражается в журнале транзакций и потому реплицируется на ведомый сервер.

...
6.  `pg_basebackup -h master_IP -U postgres -p 5432 -D /var/lib/pgsql/13/backups -Fp -Xs -P -R` - на slave. Копирует папку data на slave с работающего мастера.    
7.  Проверить содержимое файла `postgresql.auto.conf`, который должен формироваться автоматически. 
